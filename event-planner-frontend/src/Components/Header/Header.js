...
function Header({toggleSidebar, setToggleSidebar}) {
    ...
    const handleInviteClick = async () => {
        ...
        const {data} = await axios.get("/events/invites");
        for (let i = 0; i < data.length; i++) {
            array.push(<Invite key={i} type="invite" event={data[i]}/>);
        }
        setInvitesArray(array);
    ...
    const fillSearchResults = (searchResults) => {
        ...
        for (let i = 0; i < searchResults.length; i++) {
            array.push(<Link style={{textDecoration:"none"}} to={`/event/${searchResults[i].id}`}><Invite key={i} type="search" event={searchResults[i]}/></Link>);
        }
    ...
    const performLiveSearch = (searchText) => {
        let matches = events.filter((event) => {
            ...
            return (
                ...
                event.type_of_event.match(regex) ||
                event.owner_name.match(regex)
            );
        });
